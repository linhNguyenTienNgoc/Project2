...
BEGIN
    ...
    ('admin2', 'admin123', N'Nguyễn Văn Admin', 'admin@coffeeshop.com', '0901234567', 'ADMIN', 1),
...
IF NOT EXISTS (SELECT * FROM Invoice WHERE invoice_number = 'INV001')
BEGIN
    -- Insert sample invoices
    INSERT INTO Invoice (invoice_number, table_id, employee_id, order_time, payment_time, status, total_amount, payment_method) VALUES
    ('INV001', 1, 2, DATEADD(day, -2, GETDATE()), DATEADD(day, -2, GETDATE()), 'PAID', 125000, 'CASH'),
    ('INV002', 3, 2, DATEADD(day, -1, GETDATE()), DATEADD(day, -1, GETDATE()), 'PAID', 180000, 'CARD'),
    ('INV003', 5, 3, GETDATE(), NULL, 'PENDING', 95000, NULL),
    ('INV004', 2, 2, DATEADD(hour, -3, GETDATE()), DATEADD(hour, -3, GETDATE()), 'PAID', 140000, 'CASH'),
    ('INV005', 4, 3, DATEADD(hour, -1, GETDATE()), NULL, 'PENDING', 75000, NULL);

    -- Insert invoice details
    INSERT INTO InvoiceDetail (invoice_id, menu_id, quantity, unit_price, total_price) VALUES
    (1, 1, 2, 25000, 50000),  -- 2 cà phê đen
    (1, 2, 1, 30000, 30000),  -- 1 cà phê sữa
    (1, 11, 1, 55000, 55000), -- 1 tiramisu
    (2, 3, 2, 45000, 90000),  -- 2 cappuccino
    (2, 12, 2, 50000, 100000), -- 2 cheesecake
    (3, 4, 1, 50000, 50000),  -- 1 latte
    (3, 13, 1, 45000, 45000), -- 1 chocolate cake
    (4, 6, 2, 35000, 70000),  -- 2 trà sữa
    (4, 15, 2, 35000, 70000), -- 2 sandwich
    (5, 5, 1, 35000, 35000),  -- 1 espresso
    (5, 16, 1, 25000, 25000), -- 1 bánh mì bơ
    (5, 17, 1, 30000, 30000); -- 1 kem vanilla

    -- Update table status for occupied tables
    UPDATE CoffeeTable SET status = 'OCCUPIED' WHERE id IN (5);
END
GO

-- Create Indexes for better performance
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Employee_Username')
    CREATE INDEX IX_Employee_Username ON Employee(username);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Employee_Role')
    CREATE INDEX IX_Employee_Role ON Employee(role);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_CoffeeTable_Status')
    CREATE INDEX IX_CoffeeTable_Status ON CoffeeTable(status);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Menu_Category')
    CREATE INDEX IX_Menu_Category ON Menu(category);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Menu_Available')
    CREATE INDEX IX_Menu_Available ON Menu(available);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Invoice_Status')
    CREATE INDEX IX_Invoice_Status ON Invoice(status);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Invoice_OrderTime')
    CREATE INDEX IX_Invoice_OrderTime ON Invoice(order_time);

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_InvoiceDetail_InvoiceId')
    CREATE INDEX IX_InvoiceDetail_InvoiceId ON InvoiceDetail(invoice_id);
